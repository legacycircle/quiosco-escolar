...
import { FoodActions } from "@/components/foods/food-actions";
...
export default async function FoodsPage({ searchParams }: FoodsPageProps) {
  ...
  if (!tableMissing && totalCount > 0 && currentPage > totalPages) {
    redirect(`/alimentos?page=${totalPages}`);
  }
      ...
      <section className="space-y-4 lg:space-y-5">
        <div className="space-y-3 lg:flex lg:items-start lg:justify-between lg:gap-6">
          <div>
            ...
            <p className="mt-1.5 max-w-2xl text-sm text-[color:var(--brand-mid)] sm:text-base">
              Preparados del dia con costo flexible, sin control de stock y con precio visible para venta.
            </p>
          </div>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <MetricCard
            ...
            value={tableMissing ? "S/ 0.00" : formatCurrency(averageSalePrice)}
          />
        </section>
      </section>
