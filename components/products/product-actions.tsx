...
function formatProductError(message: string) {
  ...
  return buildFeedback(
    "No se pudo guardar",
    "Supabase devolvió un error al guardar el producto.",
    message
  );
}
  ...
  const submitCreate = () => {
    ...
    if (costoUnitario == null) {
      setFeedback(
        buildFeedback(
          "Costo inválido",
          "Ingresa un costo válido para calcular el costo unitario sin redondeos peligrosos."
        )
      );
      return;
    }
      ...
      void (async () => {
        try {
          ...
          setFeedback(
            buildFeedback(
              "Producto registrado",
              "El producto nuevo se guardó correctamente.",
              undefined,
              "success"
            )
          );
    ...
    if (!selectedProduct) {
      setFeedback(
        buildFeedback(
          ...
          "Selecciona un producto existente para agregar inventario."
        )
      );
      return;
    }
        ...
        try {
          ...
          setFeedback(
            buildFeedback(
              ...
              `Se agregaron ${cantidad} unidades a ${selectedProduct.nombre}.`,
              undefined,
              "success"
            )
          );
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-end gap-3">
        <button
          type="button"
          onClick={openRestock}
          className="inline-flex min-h-11 items-center justify-center rounded-full border border-[#eadcd2] bg-white px-5 text-sm font-semibold text-[color:var(--brand-dark)] transition hover:border-[color:var(--brand-mid)]"
        >
          Agregar inventario
        </button>
        <button
                  ...
                  >
                    <label className="block space-y-2 md:col-span-2">
                      ...
                      <input
                        ...
                        type="text"
                        placeholder="Ej. Galleta animalitos"
                    ...
                    <label className="block space-y-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((current) => ({ ...current, categoria: event.target.value }))
                        }
                        type="text"
                        placeholder="Galletas, bebidas, snacks..."
                        className="h-11 w-full rounded-xl border border-[#e7ddd6] bg-white px-4 text-sm text-[color:var(--brand-dark)] outline-none transition placeholder:text-[color:var(--brand-mid)]/60 focus:border-[color:var(--accent)] focus:ring-4 focus:ring-[rgba(209,7,84,0.12)]"
                      />
                    </label>
                    ...
                    <label className="block space-y-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((current) => ({ ...current, cantidad: event.target.value }))
                        }
                        type="number"
                        min="1"
                        step="1"
                    ...
                    <label className="block space-y-2">
                      <span className="text-sm font-semibold text-[color:var(--brand-dark)]">
                        {createForm.costMode === "unit" ? "Costo unitario" : "Costo total de la compra"}
                      </span>
                      <input
                        value={createForm.costValue}
                        onChange={(event) =>
                          setCreateForm((current) => ({ ...current, costValue: event.target.value }))
                        }
                        type="text"
                        inputMode="decimal"
                        placeholder="0.00"
                    ...
                    <label className="block space-y-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((current) => ({ ...current, precioVenta: event.target.value }))
                        }
                        type="text"
                        inputMode="decimal"
                  ...
                  >
                    <label className="block space-y-2 md:col-span-2">
                      ...
                      <input
                        ...
                        value={restockForm.nombre}
                        onChange={(event) =>
                          setRestockForm((current) => ({ ...current, nombre: event.target.value }))
                        }
                        type="text"
                    ...
                    <label className="block space-y-2">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRestockForm((current) => ({ ...current, cantidad: event.target.value }))
                        }
                        type="number"
                        min="1"
                        step="1"
                    ...
                    <label className="block space-y-2">
                      <span className="text-sm font-semibold text-[color:var(--brand-dark)]">
                        {restockForm.costMode === "unit" ? "Costo unitario" : "Costo total de la compra"}
                      </span>
                      <input
                        value={restockForm.costValue}
                        onChange={(event) =>
                          setRestockForm((current) => ({ ...current, costValue: event.target.value }))
                        }
                        type="text"
                        inputMode="decimal"
                        placeholder="0.00"
                    ...
                    <div className="rounded-[1.25rem] border border-[#efe3da] bg-[#fcfaf8] px-4 py-3">
                      ...
                      <p className="mt-2 text-sm font-semibold text-[color:var(--brand-dark)]">
                        {selectedProduct ? selectedProduct.nombre : "Selecciona uno de la lista"}
                      </p>
                      <p className="mt-1 text-sm text-[color:var(--brand-mid)]">
                        {selectedProduct
                          ? `${selectedProduct.categoria} · Stock actual: ${selectedProduct.stock} · Venta: ${formatCurrency(selectedProduct.precio_venta)}`
